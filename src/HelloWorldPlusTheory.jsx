...
const HelloWorldPlusTheory = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const docRef = await addDoc(collection(db, '/thhelloworld'), {
                title,
                content,
                createdAt: new Date(),
            });
    ...
    const handleEdit = async () => {
        try {
            const theoryRef = doc(db, '/thhelloworld', savedTheory.id);
...
export default HelloWorldPlusTheory;